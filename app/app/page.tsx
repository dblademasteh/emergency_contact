...
export default function Page() {
  ...
  const fetchAllData = useCallback(async () => {
    const [contactsRes, groupsRes, typesRes, homeRes, fbRes, bfpRes, logoRes, nameRes] =
      await Promise.all([
        fetch("/api/contacts"),
        fetch("/api/groups"),
        fetch("/api/types"),
    ...
    const [contactData, groupData, typeData, homeData, fbData, bfpData, logoData, nameData] =
      await Promise.all([
        ...
        nameRes.ok
          ? (nameRes.json() as Promise<{ name?: string | null }>)
    ...
    return {
      ...
      groups: groupData,
      types: typeData,
      homeImage: homeData?.image ?? null,
      facebookPageUrl: fbData?.url ?? null,
      bfpSiteUrl: bfpData?.url ?? null,
      appLogo: logoData?.logo ?? null,
      appName: nameData?.name ?? null,
    };
  }, []);
  ...
  useEffect(() => {
    ...
    // Instant first paint from cache (if any), then refresh in the background.
    ...
    fetchAllData()
      ...
      .catch(() => {
        if (cancelled) return;
        // If we already showed cached data, keep it and don't show an error.
        if (!cached) setLoadError("Couldn't load contacts.");
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/me")
      ...
      .then((data: { role?: "admin" | "user" | null } | null) => {
        ...
        setRole(role);
        // The app view is for logged-in users only; send public visitors home.
